function wholePage(userObject, bestUsersObject, userDocID) {
    ...
    var socket = io.connect("192.168.1.214:4000"); //argument = where we want to make this websocket connection to
    ...
    function multiPlayer() {
        if (isDead) {
            ...
            if (framesGameOver < 300) {
                ctx.clearRect(0, 0, maxWidth, maxHeight);
                ctx.font = "30px Consolas";
                ctx.fillStyle = "white";
                ctx.fillText("GAME OVER", maxWidth / 2 - 90, maxHeight / 2 - 90);
                framesGameOver = framesGameOver + 1;
            } else {
                sleep(2000)
                isPlaying = false;
                gameOver = false;
                ...
                if (onHallway && isPlaying) {
                    ...
                    if (isDead) {
                        ...
                        if (framesGameOver < 300) {
                            ...
                            //firebase call
                            //let db = firebase.firestore();
                            var newDeaths = userObject.deaths + 3;
                            var newKills = userObject.kills + circle.phantomKills;
                            ...
                            userObject.deaths = newDeaths
                            userObject.kills = newKills
